  ...
  <div class="viewResultContainer">
    <div class="topContainer">
      <div class="videoContainer">
        ...
        <div class="viewOriginal">
          <video id="viewOriginal" class="video-js vjs-big-play-centered" controls data-setup="{}">
            <source :src="originalVideoUrl" type="video/mp4" />
          </video>
        </div>
        <div class="viewModified" id="viewModifiedContainer">
          <video id="viewModified" class="video-js vjs-big-play-centered" controls data-setup="{}">
            <source :src="videoUrl" type="video/mp4" />
          </video>
        </div>
        <div class="labelContainer" style="marginTop:10px">
          <span class="labelTitle" v-if="viewResults.result.defended == undefined">Modified:</span>
          <el-select
            v-else
      ...
      <div class="featureContainer">
        <div class="selectContainer">
          ...
          <el-select v-model="methodValue" @change="chooseClick('one')" class="methodOne">
            <el-option
          ...
          <el-select v-model="methodDetailValue" @change="chooseClick('two')" class="methodTwo">
            <el-option
        ...
        <div class="waveform">
          ...
          <div id="originalWaveform" @click="originalSeekToTime(null)" ref="originalWaveform"></div>
        </div>
        <div style="position:relative">
          <div id="main" style="width: 550px;height:350px;marginTop:-10px"></div>
        </div>
        <div class="waveform" v-if="modifiedWaveSwitch" style="marginTop:-30px">
          <div id="modifiedTimeline" ref="modifiedTimeline"></div>
          <div id="modifiedWave" @click="modifiedSeekToTime(null)" ref="modifiedWave"></div>
        </div>
      </div>
    </div>
    <div class="bottomContainer">
      ...
      <div class="audioContainer">
        <el-slider v-model="sliderValue" :format-tooltip="formatTooltip" @input="sliderChange" />
        <div class="control">
          <svg
            t="1662145678112"
            class="icon"
    ...
    <el-dialog v-model="dialogScores" title="Model Scores" width="650px">
      <div class="dialogContainer">
        <div class="dialogScoresContainer">
          <div>Original</div>
          <div class="dialogItem" v-if="viewResults.result.original.tfn!= undefined">
            <div class="dialogTitle">TFN:</div>
            <el-input
              :style="{backgroundColor: viewResults.result.original.tfn<-0.5?'rgba(241, 76, 76, 0.1)':viewResults.result.original.tfn<0.5?'rgba(172, 175, 2, 0.1)':'rgba(60, 190, 157, 0.1)'}"
              readonly
              v-model="viewResults.result.original.tfn"
            />
          </div>
          <div class="dialogItem" v-if="viewResults.result.original.lmf!= undefined">
            <div class="dialogTitle">LMF:</div>
            <el-input
              :style="{backgroundColor: viewResults.result.original.lmf<-0.5?'rgba(241, 76, 76, 0.1)':viewResults.result.original.lmf<0.5?'rgba(172, 175, 2, 0.1)':'rgba(60, 190, 157, 0.1)'}"
              readonly
              v-model="viewResults.result.original.lmf"
            />
          </div>
          <div class="dialogItem" v-if="viewResults.result.original.bert_mag!= undefined">
            <div class="dialogTitle">MAG-BERT:</div>
            <el-input
              :style="{backgroundColor: viewResults.result.original.bert_mag<-0.5?'rgba(241, 76, 76, 0.1)':viewResults.result.original.bert_mag<0.5?'rgba(172, 175, 2, 0.1)':'rgba(60, 190, 157, 0.1)'}"
              readonly
              v-model="viewResults.result.original.bert_mag"
            />
          </div>
          <div class="dialogItem" v-if="viewResults.result.original.misa!= undefined">
            <div class="dialogTitle">MISA:</div>
            <el-input
              :style="{backgroundColor: viewResults.result.original.misa<-0.5?'rgba(241, 76, 76, 0.1)':viewResults.result.original.misa<0.5?'rgba(172, 175, 2, 0.1)':'rgba(60, 190, 157, 0.1)'}"
              readonly
              v-model="viewResults.result.original.misa"
            />
          </div>
          <div class="dialogItem" v-if="viewResults.result.original.mult!= undefined">
            <div class="dialogTitle">MulT:</div>
            <el-input
              :style="{backgroundColor: viewResults.result.original.mult<-0.5?'rgba(241, 76, 76, 0.1)':viewResults.result.original.mult<0.5?'rgba(172, 175, 2, 0.1)':'rgba(60, 190, 157, 0.1)'}"
              readonly
              v-model="viewResults.result.original.mult"
            />
          </div>

          <div class="dialogItem" v-if="viewResults.result.original.mmim!= undefined">
            <div class="dialogTitle">MMIM:</div>
            <el-input
              :style="{backgroundColor: viewResults.result.original.mmim<-0.5?'rgba(241, 76, 76, 0.1)':viewResults.result.original.mmim<0.5?'rgba(172, 175, 2, 0.1)':'rgba(60, 190, 157, 0.1)'}"
              readonly
              v-model="viewResults.result.original.mmim"
            />
          </div>

          <div class="dialogItem" v-if="viewResults.result.original.self_mm != undefined">
            <div class="dialogTitle">Self-MM:</div>
            <el-input
              :style="{backgroundColor: viewResults.result.original.self_mm<-0.5?'rgba(241, 76, 76, 0.1)':viewResults.result.original.self_mm<0.5?'rgba(172, 175, 2, 0.1)':'rgba(60, 190, 157, 0.1)'}"
              readonly
              v-model="viewResults.result.original.self_mm"
            />
          </div>
          <div class="dialogItem" v-if="viewResults.result.original.tfr_net!= undefined">
            <div class="dialogTitle">TFR-Net:</div>
            <el-input
              :style="{backgroundColor: viewResults.result.original.tfr_net<-0.5?'rgba(241, 76, 76, 0.1)':viewResults.result.original.tfr_net<0.5?'rgba(172, 175, 2, 0.1)':'rgba(60, 190, 157, 0.1)'}"
              readonly
              v-model="viewResults.result.original.tfr_net"
            />
          </div>
          <div class="dialogItem" v-if="viewResults.result.original.niat!= undefined">
            <div class="dialogTitle">NIAT:</div>
            <el-input
              :style="{backgroundColor: viewResults.result.original.niat<-0.5?'rgba(241, 76, 76, 0.1)':viewResults.result.original.niat<0.5?'rgba(172, 175, 2, 0.1)':'rgba(60, 190, 157, 0.1)'}"
              readonly
          ...
          <div class="dialogItem" v-if="labelResult.original!= undefined">
            ...
            <el-input
              :style="{backgroundColor: labelResult.original<-0.5?'rgba(241, 76, 76, 0.1)':labelResult.original<0.5?'rgba(172, 175, 2, 0.1)':'rgba(60, 190, 157, 0.1)'}"
              readonly
        ...
        <div class="dialogScoresContainer">
          <div>Modified</div>
          <div class="dialogItem" v-if="viewResults.result.modified.tfn!= undefined">
            <div class="dialogTitle">TFN:</div>
            <el-input
              :style="{backgroundColor: viewResults.result.modified.tfn<-0.5?'rgba(241, 76, 76, 0.1)':viewResults.result.modified.tfn<0.5?'rgba(172, 175, 2, 0.1)':'rgba(60, 190, 157, 0.1)'}"
              readonly
              v-model="viewResults.result.modified.tfn"
            />
          </div>
          <div class="dialogItem" v-if="viewResults.result.modified.lmf!= undefined">
            <div class="dialogTitle">LMF:</div>
            <el-input
              :style="{backgroundColor: viewResults.result.modified.lmf<-0.5?'rgba(241, 76, 76, 0.1)':viewResults.result.modified.lmf<0.5?'rgba(172, 175, 2, 0.1)':'rgba(60, 190, 157, 0.1)'}"
              readonly
              v-model="viewResults.result.modified.lmf"
            />
          </div>
          <div class="dialogItem" v-if="viewResults.result.modified.bert_mag!= undefined">
            <div class="dialogTitle">MAG-BERT:</div>
            <el-input
              :style="{backgroundColor: viewResults.result.modified.bert_mag<-0.5?'rgba(241, 76, 76, 0.1)':viewResults.result.modified.bert_mag<0.5?'rgba(172, 175, 2, 0.1)':'rgba(60, 190, 157, 0.1)'}"
              readonly
              v-model="viewResults.result.modified.bert_mag"
            />
          </div>
          <div class="dialogItem" v-if="viewResults.result.modified.misa!= undefined">
            <div class="dialogTitle">MISA:</div>
            <el-input
              :style="{backgroundColor: viewResults.result.modified.misa<-0.5?'rgba(241, 76, 76, 0.1)':viewResults.result.modified.misa<0.5?'rgba(172, 175, 2, 0.1)':'rgba(60, 190, 157, 0.1)'}"
              readonly
              v-model="viewResults.result.modified.misa"
            />
          </div>
          <div class="dialogItem" v-if="viewResults.result.modified.mult!= undefined">
            <div class="dialogTitle">MulT:</div>
            <el-input
              :style="{backgroundColor: viewResults.result.modified.mult<-0.5?'rgba(241, 76, 76, 0.1)':viewResults.result.modified.mult<0.5?'rgba(172, 175, 2, 0.1)':'rgba(60, 190, 157, 0.1)'}"
              readonly
              v-model="viewResults.result.modified.mult"
            />
          </div>

          <div class="dialogItem" v-if="viewResults.result.modified.mmim!= undefined">
            <div class="dialogTitle">MMIM:</div>
            <el-input
              :style="{backgroundColor: viewResults.result.modified.mmim<-0.5?'rgba(241, 76, 76, 0.1)':viewResults.result.modified.mmim<0.5?'rgba(172, 175, 2, 0.1)':'rgba(60, 190, 157, 0.1)'}"
              readonly
              v-model="viewResults.result.modified.mmim"
            />
          </div>

          <div class="dialogItem" v-if="viewResults.result.modified.self_mm!= undefined">
            <div class="dialogTitle">Self-MM:</div>
            <el-input
              :style="{backgroundColor: viewResults.result.modified.self_mm<-0.5?'rgba(241, 76, 76, 0.1)':viewResults.result.modified.self_mm<0.5?'rgba(172, 175, 2, 0.1)':'rgba(60, 190, 157, 0.1)'}"
              readonly
              v-model="viewResults.result.modified.self_mm"
            />
          </div>
          <div class="dialogItem" v-if="viewResults.result.modified.tfr_net!= undefined">
            <div class="dialogTitle">TFR-Net:</div>
            <el-input
              :style="{backgroundColor: viewResults.result.modified.tfr_net<-0.5?'rgba(241, 76, 76, 0.1)':viewResults.result.modified.tfr_net<0.5?'rgba(172, 175, 2, 0.1)':'rgba(60, 190, 157, 0.1)'}"
              readonly
              v-model="viewResults.result.modified.tfr_net"
            />
          </div>
          <div class="dialogItem" v-if="viewResults.result.modified.niat!= undefined">
            <div class="dialogTitle">NIAT:</div>
            <el-input
              :style="{backgroundColor: viewResults.result.modified.niat<-0.5?'rgba(241, 76, 76, 0.1)':viewResults.result.modified.niat<0.5?'rgba(172, 175, 2, 0.1)':'rgba(60, 190, 157, 0.1)'}"
              readonly
          ...
          <div class="dialogItem" v-if="labelResult.modified!= undefined">
            <div class="dialogTitle">Average:</div>
            <el-input
              :style="{backgroundColor: labelResult.modified<-0.5?'rgba(241, 76, 76, 0.1)':labelResult.modified<0.5?'rgba(172, 175, 2, 0.1)':'rgba(60, 190, 157, 0.1)'}"
              readonly
              v-model="labelResult.modified"
            />
          </div>
        </div>
        <div class="dialogScoresContainer" v-if="viewResults.result.defended!= undefined">
          <div>Defended</div>
          <div class="dialogItem" v-if="viewResults.result.defended.tfn!= undefined">
            <div class="dialogTitle">TFN:</div>
            <el-input
              :style="{backgroundColor: viewResults.result.defended.tfn<-0.5?'rgba(241, 76, 76, 0.1)':viewResults.result.defended.tfn<0.5?'rgba(172, 175, 2, 0.1)':'rgba(60, 190, 157, 0.1)'}"
              readonly
              v-model="viewResults.result.defended.tfn"
            />
          </div>
          <div class="dialogItem" v-if="viewResults.result.defended.lmf!= undefined">
            <div class="dialogTitle">LMF:</div>
            <el-input
              :style="{backgroundColor: viewResults.result.defended.lmf<-0.5?'rgba(241, 76, 76, 0.1)':viewResults.result.defended.lmf<0.5?'rgba(172, 175, 2, 0.1)':'rgba(60, 190, 157, 0.1)'}"
              readonly
              v-model="viewResults.result.defended.lmf"
            />
          </div>
          <div class="dialogItem" v-if="viewResults.result.defended.bert_mag!= undefined">
            <div class="dialogTitle">Bert-Mag:</div>
            <el-input
              :style="{backgroundColor: viewResults.result.defended.bert_mag<-0.5?'rgba(241, 76, 76, 0.1)':viewResults.result.defended.bert_mag<0.5?'rgba(172, 175, 2, 0.1)':'rgba(60, 190, 157, 0.1)'}"
              readonly
              v-model="viewResults.result.defended.bert_mag"
            />
          </div>
          <div class="dialogItem" v-if="viewResults.result.defended.misa!= undefined">
            <div class="dialogTitle">MISA:</div>
            <el-input
              :style="{backgroundColor: viewResults.result.defended.misa<-0.5?'rgba(241, 76, 76, 0.1)':viewResults.result.defended.misa<0.5?'rgba(172, 175, 2, 0.1)':'rgba(60, 190, 157, 0.1)'}"
              readonly
              v-model="viewResults.result.defended.misa"
            />
          </div>
          <div class="dialogItem" v-if="viewResults.result.defended.mult!= undefined">
            <div class="dialogTitle">MulT:</div>
            <el-input
              :style="{backgroundColor: viewResults.result.defended.mult<-0.5?'rgba(241, 76, 76, 0.1)':viewResults.result.defended.mult<0.5?'rgba(172, 175, 2, 0.1)':'rgba(60, 190, 157, 0.1)'}"
              readonly
              v-model="viewResults.result.defended.mult"
            />
          </div>

          <div class="dialogItem" v-if="viewResults.result.defended.mmim!= undefined">
            <div class="dialogTitle">MMIM:</div>
            <el-input
              :style="{backgroundColor: viewResults.result.defended.mmim<-0.5?'rgba(241, 76, 76, 0.1)':viewResults.result.defended.mmim<0.5?'rgba(172, 175, 2, 0.1)':'rgba(60, 190, 157, 0.1)'}"
              readonly
              v-model="viewResults.result.defended.mmim"
            />
          </div>

          <div class="dialogItem" v-if="viewResults.result.defended.self_mm!= undefined">
            <div class="dialogTitle">Self-MM:</div>
            <el-input
              :style="{backgroundColor: viewResults.result.defended.self_mm<-0.5?'rgba(241, 76, 76, 0.1)':viewResults.result.defended.self_mm<0.5?'rgba(172, 175, 2, 0.1)':'rgba(60, 190, 157, 0.1)'}"
              readonly
              v-model="viewResults.result.defended.self_mm"
            />
          </div>
          <div class="dialogItem" v-if="viewResults.result.defended.tfr_net!= undefined">
            <div class="dialogTitle">TFR-Net:</div>
            <el-input
              :style="{backgroundColor: viewResults.result.defended.tfr_net<-0.5?'rgba(241, 76, 76, 0.1)':viewResults.result.defended.tfr_net<0.5?'rgba(172, 175, 2, 0.1)':'rgba(60, 190, 157, 0.1)'}"
              readonly
              v-model="viewResults.result.defended.tfr_net"
            />
          </div>
          <div class="dialogItem" v-if="viewResults.result.defended.niat!= undefined">
            <div class="dialogTitle">NIAT:</div>
            <el-input
              :style="{backgroundColor: viewResults.result.defended.niat<-0.5?'rgba(241, 76, 76, 0.1)':viewResults.result.defended.niat<0.5?'rgba(172, 175, 2, 0.1)':'rgba(60, 190, 157, 0.1)'}"
              readonly
          ...
          <div class="dialogItem" v-if="labelResult.defended!= undefined">
            <div class="dialogTitle">Average:</div>
            <el-input
              :style="{backgroundColor: labelResult.defended<-0.5?'rgba(241, 76, 76, 0.1)':labelResult.defended<0.5?'rgba(172, 175, 2, 0.1)':'rgba(60, 190, 157, 0.1)'}"
              readonly
      ...
      <template #footer>
        <span class="dialog-footer">
          <el-button type="primary" @click="dialogScores=false">Confirm</el-button>
        </span>
      </template>
    </el-dialog>
    <div
      class="ztooltipResult"
...
const textOver = async (e, index, background, textBackground) => {
  ...
  if (textBackground != "#fff") {
    ...
    setTimeout(() => {
      tooltipLeft.value = triangleLeft.value - $(`.ztooltipResult`).width() / 2 + 2;
    }, 1);
  }
};
const chooseClick = t => {
...
const highlightOver = (index, background, textBackground) => {
  ...
  $(
    `.pageContainer .viewResultContainer .textContent span:nth-child(${index +
      1})`
  ).css({
